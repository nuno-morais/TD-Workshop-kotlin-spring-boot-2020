...
class BillsService(
	...
	fun findById(id: Int): Bill = billsRepository.findById(id).orElseThrow{
		NotFoundException()
	}
	...
	fun delete(id: Int) {
		findById(id).run {billsRepository.deleteById(id)}

	}

	fun create(bill: Bill): Bill = billsRepository.save(bill)

	fun update(bill: Bill): Bill = findById(bill.id!!).run{billsRepository.save(bill)}
}